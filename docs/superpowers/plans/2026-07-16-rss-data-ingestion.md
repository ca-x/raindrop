...
- Create: `src/db/migration/rss/feed_metadata.rs`
- Modify: `src/db/migration/rss/mod.rs`
- Modify: `src/db/migration.rs`
- Modify: `src/db/entities/feed.rs`
- Modify: `src/content/sanitize/mod.rs`
...
- `sanitized_content` stores exact prefix `rdsc:v1:` followed by compact canonical JSON `{"html":...,"inertImages":[...]}`. The private storage structs declare fields in that exact order; inert images are strictly increasing by `imageIndex`, have unique indexes, and retain only sanitized absolute HTTP(S) source URLs without credentials plus bounded alt/width/height metadata. Serialization names are `html` and `inertImages`; image names are `imageIndex`, `sourceUrl`, `alt`, `width`, and `height`.
- `EncodedEntryContent::from_sanitized(&SanitizedContent)` is the only encoder and owns the canonical storage string; it exposes only `as_storage_str()`. `EntryContentDetail::decode(&str)` is a separate read-only validated type with `{ html, inert_images }` getters for Task 9. There is no public or crate-visible constructor from an arbitrary `String` into the trusted encoded type.
- Encoding and decoding reject envelopes above 4 MiB, unknown prefixes/versions, malformed JSON, HTML above 1 MiB, more than 256 images, canonical image metadata above 256 KiB, alt above 4 KiB, dimensions outside `1..=16_384`, unsafe/non-absolute image URLs, duplicate/out-of-order/out-of-range `imageIndex`, metadata that does not match the indexed final `<img>`, and HTML that is not already an idempotent output of the final sanitizer. Errors are typed and redacted. No raw publisher HTML is accepted at the runtime seam.
- The storage envelope never participates in entry identity or semantic hashes. `source_content_hash` and `content_hash` are the Task 5 HTML-only values. An image-source-only envelope change updates the existing row's envelope and `updated_at` while preserving identity, immutable ordering fields, semantic hashes, and ingest generation; it counts as one updated entry. A tracking/style/input change that leaves both canonical HTML and the canonical envelope byte-identical performs no large-field rewrite.
- Add an ordered schema-evolution migration after all existing migrations. On MySQL it first inspects `information_schema` and widens `entries.sanitized_content` to `LONGTEXT` and `identity/title/author/summary/enclosure_json` to `MEDIUMTEXT` while preserving nullability, so a near-capacity legacy `TEXT` value can expand into its envelope without truncation. It then keyset-scans at most 32 rows per batch and backfills every legacy bare-HTML `sanitized_content` value to `rdsc:v1:{"html":<old>,"inertImages":[]}` with parameterized, deterministic serialization; valid already-converted envelopes are skipped so partial MySQL DDL can be retried. SQLite and PostgreSQL remain logical `TEXT` and run the same bounded backfill. Fresh databases and already-migrated databases converge to the same physical/data schema.
- The down migration makes one bounded keyset validation pass over all envelopes, refuses rollback when any row has non-empty `inertImages`, then makes a second bounded pass that restores HTML to the legacy column. Before any MySQL narrowing it uses `OCTET_LENGTH` to reject values above `TEXT` capacity; it never truncates. Up/down are safe to retry after partial MySQL DDL and are verified with bounded fixtures.
- Add a later ordered, additive feed-metadata migration with nullable `feeds.title` and `feeds.site_url`; it adds or removes only those columns, is safe to reenter after partial DDL, preserves existing feed rows, and makes fresh and upgraded schemas converge. SQLite/PostgreSQL use logical `TEXT`. MySQL uses nullable `MEDIUMTEXT` for `title`, because the parser accepts the full inclusive `MAX_TITLE_BYTES = 64 * 1024` byte budget, and nullable `TEXT` for the already bounded 4,096-byte `site_url`.
...
- Existing rows preserve ID, inserted time, ingest generation, feed sequence, and sort key. Canonical HTML+envelope equality avoids a large-field rewrite; an envelope-only inert-image change updates the existing row without changing semantic hashes or generation. `sort_at_us` is derived from checked `published_at_us` or insertion time once and never changes.
- `PersistFeed` owns the normalized nullable `ParsedFeed.title`, sanitized nullable `ParsedFeed.canonical_url`, exact final validator URL, and optional `OpaqueValidator` ETag/Last-Modified values before the transaction begins. The same fenced feed transaction persists exact `title`/`site_url` nullable values and encodes validators to canonical storage text; repository readback decodes validators before the next fetch. Corrupt/unknown validator storage is a typed repository error and is never forwarded as a header. Network, parsing, normalization, and content processing never enter the transaction.
- New entries are inserted in parsed order with parameterized multi-value statements containing at most 32 rows each. PostgreSQL placeholders remain contiguous across every row; SQLite uses 19 bindings per row (608 for a full batch), below the conservative parameter ceiling; MySQL never uses broad `INSERT IGNORE`. Existing-entry updates remain row-oriented.
- A failed insert batch rolls back the whole feed persist: no partial entry rows, generation/sequence-head advance, feed metadata/validator update, refresh-run completion, or lease release may survive. Every row in one persist shares the same new ingest generation, sequence assignment preserves parsed order, replay is idempotent, and pre-existing `(feed_id, identity_hash)` hits still compare full typed identity before any insert.
...
Write RED/GREEN tests for deterministic `rdsc:v1:` bytes, round-trip of HTML plus indexed inert images, strict sanitizer/image/index validation, rejection of corrupt/oversized/unknown envelopes, legacy bare-HTML backfill, retry after a partially widened MySQL schema, fail-closed down migration, image-source-only changes preserving the existing HTML hashes, feed-metadata additive upgrade/up-down-up, and MySQL physical column types including the 64 KiB feed-title capacity. Extend `rss_migrations` so both Task 7 migrations participate in up/down/up and named schema verification.
...
Run RED/GREEN cycles for: first 60 inserts; feed title/site URL first persistence, later change, nullable values, and stale-token no-write; second identical refresh inserts zero; tracking-only change updates zero; a real content change under GUID/URL or unchanged fingerprint inputs updates one without changing identity fields/state; accepted content-only/title/date/enclosure fallback changes insert a new identity; concurrent persists leave one identity row; both kind/text are checked on hash collision; non-UTF-8 validator bytes survive database store/reload/decode on all three backends; 65 new entries execute as 32+32+1 parameterized batches while preserving parsed-order sequences and one generation; replay is idempotent; an induced second-batch failure proves full rollback; no-new-entry refresh does not increment the counter; a stale token writes nothing. Keep the SQLite contract local and the PostgreSQL/MySQL variants on the existing serial CI filters.
