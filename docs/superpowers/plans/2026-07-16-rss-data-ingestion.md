...
- Add an ordered schema-evolution migration after all existing migrations. Before runtime decoding is enabled, it backfills every legacy bare-HTML `sanitized_content` value to `rdsc:v1:{"html":<old>,"inertImages":[]}` with parameterized, deterministic serialization; valid already-converted envelopes are skipped so a failed MySQL DDL attempt can be retried. On MySQL it then inspects `information_schema` and widens `entries.sanitized_content` to `LONGTEXT` and `identity/title/author/summary/enclosure_json` to `MEDIUMTEXT` while preserving nullability; SQLite and PostgreSQL remain logical `TEXT`. Fresh databases and already-migrated databases converge to the same physical/data schema.
- The down migration first decodes every envelope, refuses rollback when any row has non-empty `inertImages`, and restores bounded envelope HTML to the legacy column. Before any MySQL narrowing it uses `OCTET_LENGTH` to reject values above `TEXT` capacity; it never truncates. Up/down are safe to retry after partial MySQL DDL and are verified with bounded fixtures.
