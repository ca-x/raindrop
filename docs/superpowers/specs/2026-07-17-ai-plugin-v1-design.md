...
#### 7.3.1 Provider adapter matrix

首版 `providerKind` 固定为以下四个 additive enum；provider binding 只引用 provider ID，插件看不到 kind、base URL、credential 或供应商 DTO：

| `providerKind` | wire contract | 首版用途 |
| --- | --- | --- |
| `ANTHROPIC_MESSAGES` | Anthropic Messages-compatible（Claude 格式） | Claude 官方 API 或通过 URL policy 的兼容端点 |
| `OPENAI_RESPONSES` | OpenAI Responses API | OpenAI 原生 Responses structured generation |
| `OPENAI_CHAT_COMPLETIONS` | OpenAI Chat Completions-compatible API | OpenAI Chat Completions、兼容网关和本地模型服务 |
| `GOOGLE_GEMINI` | Google Gemini native API | Gemini 官方原生 generate content 接口 |

四个 adapter 都消费同一 canonical `AiRequest`，并只返回 canonical `AiResponse`。adapter 独占供应商请求构造、认证/version header、stream/non-stream decoding、structured-output 映射、usage/finish reason 归一化和 provider error 分类；供应商响应在进入 core 前按不可信输入解析，未知字段忽略、缺失必需字段 fail closed。adapter 不实现业务 prompt、artifact schema、MCP transport 或 tool loop。

管理员配置 provider 时必须显式选择 `providerKind`；kind 创建后不可原地改写，切换协议通过新 provider ID 完成，避免已有 job/config 的 transport 语义漂移。自定义 base URL 必须经过 scheme、credential、DNS/IP、redirect 和 connected-peer policy 校验；secret 仍只存在 provider core。模型能力由 adapter 探测或配置为 canonical capability flags，业务层不得按供应商名称分支。
...
| Provider | 四类 adapter contract fixture（Anthropic Messages/OpenAI Responses/OpenAI Chat Completions/Google Gemini）；secret 不进入 ABI/log/error；quota/rate/token/cost；timeout；invalid JSON/schema；usage/finish/error 归一化；provider idempotency header |
...
- Anthropic Messages-compatible、OpenAI Responses、OpenAI Chat Completions-compatible 和 Google Gemini 四类 adapter 均通过 canonical request/response、structured output、usage、finish reason、error/redaction 和 timeout contract tests；业务层没有供应商 DTO 或名称分支。
