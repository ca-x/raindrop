...
| `RAINDROP_DATABASE_URL` | `database_url` | 未设置。支持 `sqlite://`、`postgres://`、`postgresql://`、`mysql://`。存在时直接进入 ready 启动路径；不存在时进入设置向导。 |
...
| `RAINDROP_BOOTSTRAP_ADMIN_EMAIL` | `bootstrap_admin.email` | 未设置。可选；首尾空白会被移除，空值按未提供处理。 |
...
环境变量和 TOML 都没有提供数据库 URL 时，程序进入设置模式：

1. 每次进程启动都会生成新的 setup token，并输出到标准错误。
2. 数据库检查和完成设置都必须通过 `X-Setup-Token` 提交该 token，包括 loopback 请求。
3. 向导默认填写 `sqlite://data/raindrop.db?mode=rwc`，也可以改为 PostgreSQL 或 MySQL URL。
...
设置向导在数据目录中创建同目录临时文件，写入并 `fsync` 后，在 Unix 上设置为 `0600`，再重命名为 `config.toml` 并同步目录。重命名或目录同步失败时，会删除临时或目标文件并回滚刚创建的管理员。非 Unix 平台没有 POSIX `0600`，需要使用平台 ACL 保护数据目录和配置文件。

向导只新增或更新 `database_url`，已有 TOML 字段会保留。数据库 URL 以明文保存在 `config.toml`，文件权限和备份访问控制仍由管理员负责。

### 环境托管

只要 `RAINDROP_DATABASE_URL` 或 TOML `database_url` 存在，程序就连接数据库并运行迁移。数据库没有用户时，必须同时提供管理员用户名和密码；缺少任一项会使启动失败。管理员创建成功后，已有用户不会在后续启动中被覆盖，但部署环境仍会持有该密码，应及时删除。
...
相对 SQLite 路径以进程工作目录为基准，不以 `RAINDROP_DATA_DIR` 为基准。使用绝对数据目录时，应同时使用绝对 SQLite URL，并提前创建父目录。
