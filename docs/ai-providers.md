...
本文记录当前已经实现的 AI provider 内部核心。它包含三数据库记录、独立密钥环、四类非流式协议 adapter、SSRF-safe HTTPS transport 和统一 `ProviderClient`。通用 content job/attempt/artifact 持久化 core 也已经实现，但尚未把 job worker、provider reservation/execution、官方 Wasm 插件、摘要/翻译入口、管理 API/UI 或 MCP 组合进 runtime；因此配置密钥仍不会自动发起模型请求。
...
`max_concurrency`、`requests_per_minute`、input token ceiling、cost rate 和 per-request cost ceiling 已持久化并经过跨数据库校验，但当前 provider client 不充当 scheduler。`ContentRepository` 已提供数据库记录系统、幂等 enqueue、每用户运行并发、lease/fencing、attempt、retry/unknown outcome 和 immutable artifact 原子终态；后续 worker 必须先持有有效 `ContentJobClaim`，再做 provider quota/cost reservation 并调用 `ProviderClient`，且只能通过 `complete_success` / `complete_failure` 提交结果。HTTP route、Reader、lifecycle、Wasm guest 或 MCP server 都不得绕过该 claim 直接调用 provider。
...
- content worker 与 provider quota/cost reservation、官方插件执行、current/stale read composition、摘要和翻译；
