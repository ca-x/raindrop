...
- 设置的「阅读」页可选择衬线、无衬线或已上传的自定义字体，支持调整字号和恢复默认。
- 默认阅读正文从约 17px 调大到 20px，字号可在 85%–130% 之间调整。
- 阅读时可用 `+` / `-` 放大、缩小文本，`0` 恢复默认，也支持 Ctrl/Cmd 组合键；设置会自动保存，输入框和弹窗内不会误触。

## v0.4.20

- Provider 支持删除和修改接口类型，修改类型会保留已保存的凭据，删除后仍可关闭 AI 或切换翻译服务。
- 修复已保存或未启用的 Provider 获取模型失败的问题，获取时使用当前填写的类型和地址，无需重复输入凭据。
- Provider 设置重新分组，补充删除确认、明确的错误提示和手机布局，修改连接信息后自动取消旧请求。
- 修复 MySQL 数据库迁移回滚失败的问题。

## v0.4.19

- 修复刷新记录和闲置事件无限积累的问题：保留最近 7 天诊断记录，后台分批清理过期数据。
- 新增「设置 → 数据库」，显示真实存储占用与文章总数，支持后台压缩和回收空间，关闭设置后仍继续执行。
- 新增可选的 RSS 已读文章自动清理，默认关闭。开启后可设置保留天数，保护所有用户的未读、收藏及带 AI 处理记录的文章，并防止刷新时重新导入已清理文章。
- 清理每批处理 100 条并释放写连接；阅读使用独立连接，VACUUM 期间写入可能短暂等待。详见[数据库维护说明](docs/database-maintenance.md)。

## v0.4.18

- 减少大量订阅和长期刷新历史带来的加载开销，连续刷新通知会合并处理，避免反复读取整个列表。
- 文章列表遇到临时网络或服务端错误时会自动重试，后台恢复后可继续翻页，已有内容和阅读位置保持稳定。
- 来源栏底部集中显示刷新状态，快捷键说明可按需展开，长订阅名称和文章加载失败提示更清楚。
- 设置界面与备份数据按需加载，减少打开阅读器时的请求；设置下载失败时仍可继续阅读。

## v0.4.17

- 重新整理设置与订阅弹窗的标题区、底部操作区和滚动焦点预留，桌面与移动端都保持稳定间距，键盘焦点不再容易被固定操作栏遮挡。
- 阅读器空状态新增统一的线性图标和明确操作入口，队列与文章占位调整为稳定居中；深色正文链接增加非颜色提示，提升辨识度与无障碍表现。
- 设置分区切换和 AI 模型列表加载使用轻量、可打断的状态过渡，并在减少动态效果偏好下自动退化为纯透明度反馈。
...
- 缓存晚于服务端响应到达时不会覆盖权威数据；订阅分页在首批快速呈现后使用更大批次完成后台校准，减少大量 RSS 的往返次数。
- 订阅来源树缓存分组、未读总数和选中来源查找只在对应数据变化时重新计算，大量订阅下滚动和阅读状态更新更顺畅；长来源树由浏览器按需绘制并保持稳定行高。

## v0.4.14

- 阅读正文新增随滚动同步的阅读进度，离开顶部后工具栏以轻阴影自然浮起，回到顶部后自动收拢层次。
- “显示新文章”会使用可打断的原生平滑滚动回到队首；减少动态效果偏好下保持瞬时，键盘翻页与高频导航继续不加动画。
- 主次按钮补充轻量表面阴影和按下内凹反馈，阅读字体与主题选项也会即时响应按压。

## v0.4.13

- 新增沉浸式阅读：来源栏与文章队列自动隐藏，左右边缘悬停或聚焦即可临时唤回，正文始终保持原位。
- 完善全键盘阅读：支持 `F`、`Escape`、`[`、`]`、`Space`、`Shift+Space` 与 `O`，并避开输入框、原生控件和输入法组合状态。
- 优化阅读浮层、焦点恢复与中等宽度布局，适配减少动态效果和减少透明度偏好；同步修复前端依赖审计与发布检查。

## v0.4.12

- 大量订阅会逐页加载并立即显示，单个 Feed 刷新完成后通过实时事件独立校准；列表在后台同步或部分请求失败时仍可继续浏览和操作。
- 文章队列先显示首批结果，滚动到底部时继续分页加载；新到文章保留在待合并区域，不会打乱当前阅读位置。
- 来源栏和文章队列补充低干扰进度、明确重试与未读筛选，刷新状态、未读计数和空状态更紧凑清晰。

## v0.4.11

- 修复订阅源行尾“全部已读”钩在未读计数暂时为零或尚未校准时无法点击的问题；操作会使用服务端最新快照，避免漏掉刚到达的未读文章。

## v0.4.10

- 修复批量标记为已读在提交后仍长时间等待的问题；当前列表会立即更新，未读数与分类关系在后台校准，快照之后到达的文章仍保持未读。

## v0.4.9

- 修复阅读器缓存可能保存带查询凭据的 Feed 与文章网址；缓存现在只保留恢复列表所需字段，并在切换账号或退出时跨标签页失效。
- 后台校准只在订阅和当前队列同时成功后刷新缓存有效期；搜索结果、未确认操作和过期请求不会覆盖完整队列或已提交状态。

## v0.4.8

- 阅读器会按用户缓存订阅、分类和最近来源的文章队列；重新打开页面时先恢复可用内容，再由服务端结果在后台增量对齐，弱网失败也不会清空已有列表。
- 订阅站点图标改为私有缓存并在过期窗口后台更新，减少重复打开页面时的批量拉取与图标闪现。

## v0.4.7

- 修复 CPU 忙碌时插件运行超时可能被后续清理回调覆盖为通用异常的问题，确保超时分类稳定且不会偶发误报。

## v0.4.6

- 修复 SQLite 部署在启动或后台任务繁忙时，文章队列偶发加载失败、重开后恢复的问题。
- 升级前端路由、HTML 清洗与插件运行时依赖，修复已披露的上游安全问题。

## v0.4.5

- 编辑订阅时可以直接修改 Feed 地址；更换来源会保留订阅名称、分类与排序，并安全切换到新来源的内容和刷新状态。
- 来源树中的订阅支持拖放到分类或“未分类”，拖动目标、保存中与结果状态均有明确反馈；分类选择器继续作为键盘和触屏操作入口。

## v0.4.4

- 修复 S3 与 WebDAV 目标实际保存成功后，前端因时间字段格式不匹配而误报失败、列表不显示，随后同名重试又提示冲突的问题；已有目标会在升级后正常显示。
- 备份目标、计划与历史记录统一使用 RFC 3339 时间；“计划”页可选择一个或多个已启用目标立即手动备份，也可继续保存定时计划。
