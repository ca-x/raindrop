...
当前 binary 会内嵌、验签并编译官方 `raindrop.ai-content@1.0.0` Wasm Component。完成设置后，进程会把该 installation 幂等同步到数据库；配置 `RAINDROP_PROVIDER_SECRET_KEYS` 时启动真实的 Provider/Wasm 内容 worker，未配置时保持惰性，不生成临时密钥，也不影响 RSS。用户可以在设置的 AI 分页中管理 Anthropic Messages-compatible、OpenAI Responses、OpenAI Chat Completions-compatible 和 Google Gemini Provider，再从 Reader 手动生成摘要或翻译。Credential 只写入，不会从 API 或界面回读；AI sidecar 不替换正文，关闭、切换或执行失败都不会阻塞原文阅读。自动入队、Feed 生命周期投递、MCP transport、插件管理 UI 和第三方插件分发仍是后续工作。
...
要启用 AI Provider 创建、credential 轮换和新的摘要/翻译任务，还需注入独立的 `RAINDROP_PROVIDER_SECRET_KEYS`。启动后在“设置 > AI”中创建 Provider 并保存摘要或翻译配置。完整格式、轮换和备份规则见 [AI Provider 运维合同](docs/ai-providers.md)。
