...
it("renames, moves, opens, and deletes the selected feed", async () => {
  ...
  const dialog = screen.getByRole("dialog", { name: "Manage current feed" })
  const site = within(dialog).getByRole("link", { name: "Open site" })
...
it("uses the current page for a feed site when that preference is selected", () => {
  ...
  expect(screen.getByRole("link", { name: "Open site" })).not.toHaveAttribute(
    "target",
  )
})
